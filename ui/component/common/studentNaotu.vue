  ...
  <div class="tab_wrapper">
    <div class="header_wrapper">
      ...
      <div class="header_middle">
        <h4
          @dblclick="naotuEditorTitle"
          v-if="naotuTitleSwitch"
          class="text_ellipsis"
        >{{naotuTitle}}</h4>
        <input
          v-focus
          ...
          class="inputBox"
          v-if="!naotuTitleSwitch"
          @blur="naotuEditorInput"
          @keyup.enter="naotuEditorInput"
          v-model="naotuTitle"
        >
      </div>
...
<script>
import homeWork from "../page/student/homeWork.vue";
import qs from "qs";
...
export default {
  ...
  created() {
    this.bus.$on("changeNaotu", naotu => {
      ...
      this.isTeacherFlag = naotu.is_teacher; //把打开的是教师端的脑图存起来
    });
  ...
  methods: {
    naotuStudyWork() {
      ...
      if (!this.naotuId && oldNaotu !== newNaotu) {
        this.$confirm("是否需要保存脑图?", "提示", {
          ...
          .catch(() => {
            this.$global.elMessage.message("保存失败!", "warning", 1000, this);
    ...
    naotuSaveNode(flag) {
      ...
      if (Number(this.isTeacherFlag) === 1) {
