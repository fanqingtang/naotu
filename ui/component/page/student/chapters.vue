<template>
  <div id="naotu_chapters_wrapper">
    <div class="naotu_chapters" v-for="(item,index) in chaptersData" :key="index">
      ...
      <p class="naotu_desc" :class="[item.is_teacher?'teacher_active':'']">
        ...
        <img src="public/js/naotu/images/newNaotu.png" alt="新脑图" v-if="!item.is_read && item.is_teacher">
      </p>
    ...
    methods: {
      ...
      naotuOpen(item) {
        ...
        this.$axios
          ...
          .then(res => {
            let { data: updateIsRead } = res,
              code = updateIsRead.code;
            if (code == 200) {
